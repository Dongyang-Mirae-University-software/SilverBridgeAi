...
#
# Fire  모델 (mAP50 낮음, 약한 모델)
#   → conf 낮게 설정해 실제 불/연기를 놓치지 않도록 함
#   → 오감지는 아래 Temporal Tracker 에서 걸러냄
# Knife 모델 (mAP50 높음, 강한 모델)
#   → conf 높게 설정해 배경 오인식 차단
# ==========================================
CONF_FIRE  = 0.40
CONF_KNIFE = 0.55
...
# 이 픽셀² 미만 박스는 노이즈로 제거 (너무 작은 감지 무시)
MIN_AREA_FIRE  = 2000
MIN_AREA_KNIFE = 1500
...
#
# Fire  (약한 모델): 기준 완화 → 인식률 유지
# Knife (강한 모델): 기준 강화 → 오감지 차단
# ==========================================
HISTORY_SIZE = 8   # 최근 몇 프레임을 기억할지

# Fire : 8프레임 중 4회(50%) 이상 + 평균 신뢰도 0.43 이상
FIRE_FREQ_RATIO = 0.50
FIRE_AVG_CONF   = 0.43

# Knife: 8프레임 중 5회(62%) 이상 + 평균 신뢰도 0.55 이상
KNIFE_FREQ_RATIO = 0.62
KNIFE_AVG_CONF   = 0.55
